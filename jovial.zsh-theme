...
@jov.prompt-rust-version() {
    if @jov.rev-parse-find "Cargo.toml"; then
        local rust_prompt_prefix="${JOVIAL_PALETTE[conj.]}using "
        local rust_prompt="%F{170}rust${JOVIAL_PALETTE[normal]}"
        echo "${rust_prompt_prefix}${rust_prompt}"
    fi
}
